...
The full set of available blocks is in the `/blocks/` directory: `search-bar`, `search-results`, `search-property`, `search-path`, `search-date-range`, `search-tags`, `details-renditions`, `details-preview`, `details-property`, `collection`, `collections`, and more.
