...
class SearchService {
  ...
  collectFormData(formId) {
    ...
    const inputs = document.querySelectorAll(`[form="${formId}"], form#${formId} input, form#${formId} select, form#${formId} textarea`);
  ...
  getInputValue(input) {
    ...
    switch (type) {
      ...
      case 'select-multiple':
        return Array.from(input.selectedOptions).map(option => option.value).filter(v => v);
      default:
  ...
  getPredicateId(inputName) {
    ...
    return input?.getAttribute('data-asc-predicate-id') || null;
  }

  hasValidPredicateSupport(predicateId, formData) {
    // Find inputs with for="${predicateId}" and check if they have values
    const supportingInputs = document.querySelectorAll(`[for="${predicateId}"]`);
  ...
  adjustFormData(formData) {
    ...
    adjusted.forEach((value, name) => {
      if (name.endsWith('daterange.upperBound') && typeof value === 'string' && !value.endsWith('T23:59:59.999Z')) {
  ...
  async submitSearch(formData) {
    try {
      ...
      const preprocessedQuery = this.config.preprocessQuery ? this.config.preprocessQuery(queryParams) : queryParams;
      ...
      document.dispatchEvent(new CustomEvent('asc:search:error', {
...
export default new SearchService(asc.services.search);
