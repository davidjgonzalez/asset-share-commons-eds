 ...
 *    },
 *    collections: {
 *      'collection1': ['asset1', 'asset2'],
 *      'collection2': ['asset3', 'asset4'],
 *    },
 *    ...
 * }
 * 
...
class Storage {
    ...
    /**
     * Migrates anonymous user data to the current user.
     * @param {Object} user - User object with getId() method
     */
    migrateToUser(user) {
        const userId = user.getId ? user.getId() : user;
        const anonymousData = this._getUserData(ANONYMOUS);
        ...
        if (anonymousData) {
            ...
            // Merge collections
            if (anonymousData.collections) {
                userData.collections = {
                    ...userData.collections,
                    ...anonymousData.collections
                };
            }

            // Merge other data (excluding 'user' to preserve user identity)
            Object.keys(anonymousData).forEach(key => {
                if (key !== 'user' && key !== 'collections') {
                    userData[key] = anonymousData[key];
                }
            });
            ...
            this.storage.removeItem(`${ROOT}:${ANONYMOUS}`);

            // Update current user
            this._setCurrentUserId(userId);
        }
    }
    ...
    clearUserData() {
        const userId = this._getCurrentUserId();
        this.storage.removeItem(`${ROOT}:${userId}`);
        this.init(); // Reinitialize
    }

    /**
     * Clears all ASC data (global and all users).
     */
    clearAll() {
        const keysToRemove = [];
        for (let i = 0; i < this.storage.length; i++) {
            const key = this.storage.key(i);
            if (key && key.startsWith(ROOT)) {
